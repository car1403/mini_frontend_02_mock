from typing import Any

import httpx


BACKEND_URL = "http://127.0.0.1:8000"
REQUEST_TIMEOUT = 15.0


class BackendAPIError(Exception):
    """관리자 화면이 백엔드와 통신할 때 발생하는 오류입니다."""

    pass


def request(
    method: str,
    path: str,
    json: dict[str, Any] | None = None,
):
    """백엔드에 HTTP 요청을 보내고 JSON 응답을 반환합니다."""

    try:
        response = httpx.request(
            method,
            f"{BACKEND_URL}{path}",
            json=json,
            timeout=REQUEST_TIMEOUT,
        )
    except httpx.RequestError as error:
        raise BackendAPIError("백엔드 서버에 연결할 수 없습니다.") from error

    # 400, 404, 500처럼 오류 상태 코드가 오면 이해하기 쉬운 오류로 바꿉니다.
    if response.is_error:
        try:
            detail = response.json().get("detail", "알 수 없는 오류")
        except ValueError:
            detail = response.text
        raise BackendAPIError(f"백엔드 요청 실패: {detail}")

    try:
        return response.json()
    except ValueError as error:
        raise BackendAPIError("백엔드가 올바른 JSON을 반환하지 않았습니다.") from error


def create_real_data(sensor_data: dict):
    """입력한 센서 데이터를 백엔드로 전송합니다."""

    return request("POST", "/real-data", json=sensor_data)


def get_recent_real_data(limit: int = 20):
    """Supabase에 저장된 최근 센서 데이터를 조회합니다."""

    return request("GET", f"/real-data/recent?limit={limit}")
