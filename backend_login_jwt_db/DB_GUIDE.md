...
RLS까지 적용한 다음 단계 예제는 `backend_login_jwt_db_rls` 폴더에 있습니다.

## 2. 환경 변수 만들기

`backend_login_jwt_db` 폴더에 `.env` 파일을 만듭니다.
...
| `frontend_login_jwt_db/app_pages/02_signup.py` | 회원가입 화면 |
| `frontend_login_jwt_db/core/auth.py` | 로그인 결과와 JWT 보관 |
