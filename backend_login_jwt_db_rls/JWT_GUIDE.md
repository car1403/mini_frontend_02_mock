...
> 이 폴더는 DB 연동 버전입니다. 고정된 `id01 / pwd01` 예제 대신 Supabase `customers` 테이블의 회원으로 로그인합니다. 실제 실행 순서는 [DB_GUIDE.md](DB_GUIDE.md)를 먼저 확인하세요. 아래 문서의 `id01` 값은 JWT 구조를 설명하기 위한 예시 사용자 ID입니다.
...
기존 `backend_login`에서는 로그인 요청에 성공해도 사용자 정보만 반환했습니다.
...
프론트엔드의 `frontend_login_jwt/core/api_client.py`에서는 다음 코드로 헤더를 만듭니다.
...
`frontend_login_jwt`는 응답으로 받은 `access_token`을 Session Storage에 저장합니다.
