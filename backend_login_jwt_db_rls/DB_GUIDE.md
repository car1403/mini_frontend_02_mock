...
> 이 폴더는 RLS를 활성화한 다음 단계 예제입니다. JWT와 DB 로그인만 먼저 학습하려면 `backend_login_jwt_db`를 사용하세요.
...
`backend_login_jwt_db_rls` 폴더에 `.env` 파일을 만듭니다.
...
| `frontend_login_jwt_db_rls/app_pages/02_signup.py` | 회원가입 화면 |
| `frontend_login_jwt_db_rls/core/auth.py` | 로그인 결과와 JWT 보관 |
