...
> 이 폴더는 RLS를 활성화한 다음 단계 예제입니다. JWT와 DB 로그인만 먼저 학습하려면 `login_jwt_db/backend`를 사용하세요.
...
실제 `schema.sql`에는 frontend의 직접 접근을 막기 위한 RLS 설정도 포함되어 있습니다.

## 2. 환경 변수 만들기

`login_jwt_db_rls/backend` 폴더에 `.env` 파일을 만듭니다.
...
| `login_jwt_db_rls/frontend/app_pages/02_signup.py` | 회원가입 화면 |
| `login_jwt_db_rls/frontend/core/auth.py` | 로그인 결과와 JWT 보관 |
